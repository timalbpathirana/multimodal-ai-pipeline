...
function validateScript(script) {
  ...
  if (hookWords.length > 8) {
    reasons.push(`hook is ${hookWords.length} words (max 8)`);
  }
  if (!/\d/.test(script.hook || '')) {
...
function constrainHookFromSignal(signal) {
  ...
  const numMatch = sentence.match(/\d+\.?\d*\s*(%|per cent|basis points|\$[\d,]+[km]?)/i);
  ...
  const hook = words.slice(start, start + 7).join(' ').replace(/[.,;:]+$/, '');
