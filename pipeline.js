...
const { generateScript, generateScriptWithRetry, generateOverviewScript } = require('./src/llm/claude');
const { extractSignals, NoHighSignalError } = require('./src/llm/signals');
const { markSeen } = require('./src/llm/dedup');
...
function printScriptSummary(script, signal) {
  ...
  console.log(divider);
  if (signal) {
    console.log(`  Signal  : [${signal.metricType}] "${signal.value}" (finalScore=${signal.score ?? signal.preScore})`);
    console.log(divider);
  }
  console.log(`  HOOK    : ${script.hook}`);
  console.log(`  INSIGHT : ${script.insight}`);
  console.log(`  IMPACT  : ${script.impact}`);
  const totalWords = `${script.hook} ${script.insight} ${script.impact}`.trim().split(/\s+/).length;
  console.log(divider);
  console.log(`  Total words: ${totalWords}  (~${Math.round(totalWords / 3)}s spoken)`);
  console.log(`${divider}\n`);
}

async function runPipeline() {
  if (STOP_AFTER) console.log(`[pipeline] PIPELINE_STOP_AFTER=${STOP_AFTER} — will exit early after this stage`);
  ...
  const [rssResult, ...ytResults] = await Promise.allSettled([fetchRssArticles(2), ...ytTasks]);
  ...
  for (const [i, ytResult] of ytResults.entries()) {
    if (ytResult.status === 'fulfilled') {
      ...
      console.warn(`[pipeline] YouTube ingestion failed for channel ${VIDEO_FEEDS[i]} (continuing):`, ytResult.reason.message);
    }
  }
  if (VIDEO_FEEDS.length === 0) console.log('[pipeline] YouTube skipped (VIDEO_FEEDS is empty)');
  if (contentItems.length === 0) throw new Error('[pipeline] No content ingested — aborting');
  console.log(`[pipeline] Ingested ${contentItems.length} content items (${rssCount} from RSS, ${ytCount} from YouTube)`);
  ...
  try {
    ...
    console.log(`[pipeline] Top signal: type=${topSignal.metricType} value="${topSignal.value}" finalScore=${score}`);
    script = await generateScriptWithRetry(topSignal);
    usedSignal = topSignal;
  } catch (err) {
    if (err instanceof NoHighSignalError) {
      console.warn(`[pipeline] No high-signal content today (${err.message}) — generating general overview`);
  ...
  console.log(`[pipeline] Script generated: hook="${script.hook}" (${hookWordCount} words)`);
  ...
  if (STOP_AFTER === 'voice') {
    console.log(`[pipeline] Done (voice only — video skipped). Voice: ${voicePath}`);
